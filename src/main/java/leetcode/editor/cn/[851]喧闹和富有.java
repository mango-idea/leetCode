...
// 
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 167 👎 0
...
class LoudAndRich {
    ...
    class Solution {
        public int[] loudAndRich(int[][] richer, int[] quiet) {
            ...
            Map<String, List<Integer>> richMap = new HashMap<>(quiet.length);
            for (int i = 0; i < richer.length; i++) {
                int[] rich = richer[i];
                richMap.putIfAbsent(String.valueOf(rich[0]), Collections.singletonList(rich[0]));
                if (richMap.containsKey(String.valueOf(rich[1]))) {
                    List<Integer> mRich = richMap.get(String.valueOf(rich[1]));
                    mRich.add(rich[0]);
                    richMap.put(String.valueOf(rich[1]), mRich);
                } else {
                    List<Integer> putRich = new ArrayList<>();
                    putRich.add(rich[0]);
                    putRich.add(rich[1]);
                    richMap.put(String.valueOf(rich[0]), putRich);
                }
            }
            return ans;
        }
