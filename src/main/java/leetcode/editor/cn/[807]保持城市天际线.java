...
class MaxIncreaseToKeepCitySkyline {
    ...
    class Solution {
        ...
        public int maxIncreaseKeepingSkyline(int[][] grid) {
            ...
            for (int i = 0; i < _n; i++) {
                for (int j = 0; j < _n; j++) {
                    ans += def(grid[i][j], i, j);
                }
            }
            return ans;
        }

        private int def(int num, int x, int y) {
            int rMax = 0;
            int cMax = 0;
            for (int i = 0; i < _n; i++) {
                rMax = Math.max(rMax, _grid[x][i]);
            }
            for (int j = 0; j < _n; j++) {
                cMax = Math.max(cMax, _grid[j][y]);
            }
            return Math.min(rMax, cMax) - num;
        }
    }
